        ...
        if (payload.type === "block_actions") {
            switch(payload.actions[0].action_id) {
                case "cf_repo_select_action":
                    cf_modal.update_with_env_options(payload);
                    break;
                case "cf_env_select_action":
                    cf_modal.update_with_release_options(payload);
                    break;
                case "cf_release_select_action":
                    cf_modal.update_with_submission(payload);
                    break;
            }
        }
    } catch(err) {
...
slackInteractive.viewSubmission(VIEW_SUBMISSION_CALLBACK_ID, async (payload, respond) => {
    try {
