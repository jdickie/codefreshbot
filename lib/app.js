...
slackInteractive.viewSubmission(VIEW_SUBMISSION_CALLBACK_ID, async (payload) => {
    try {
        ...
        console.log(`submission url hit with:\n ${JSON.stringify(payload)}`);
        const metaParts = payload.view.private_metadata.split(':');
        console.log(`Proj: ${metaParts[0]} \n tag: ${metaParts[1]} \n env: ${metaParts[2]}`);
            ...
            blocks: [
                {
                    ...
                    text: {
                        type: 'plain_text',
                        text: `Running ${metaParts[0]} for ${metaParts[1]}`
                    }
                },
                {
                    type: 'section',
                    text: {
                        type: 'plain_text',
                        text: `htps://g.codefresh.io/build/${result}`
                    }
                }
            ]
        });
...
app.use('/deploy_git_tag', async (req, res) => {
    ...
    try {
        console.log(`deploy_git_tag hit with\n ${JSON.stringify(req.body)}`);
        ...
        console.log(`VIEW_ID: ${VIEW_SUBMISSION_CALLBACK_ID}`);
